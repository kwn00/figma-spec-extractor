...
| 요소 | 동작 | 다음 화면 | 유형 |
|---|---|---|---|
| 회선 카드 | 선택 상태로 전환 | — | — (이동 없음) |
| [다음] | 약관 화면으로 이동 | 화면 2 | 인앱 (프로토타입 링크) |
| `←` | 정의 안 됨 | ? | 정의 안 됨 |
...
| 요소 | 동작 | 다음 화면 | 유형 |
|---|---|---|---|
| 동의 체크박스 | 체크 시 [다음] 활성화 | — | — (이동 없음) |
| [약관 전문 보기] | 정의 안 됨 (연결된 프레임 없음) | ? | 정의 안 됨 |
| [다음] | 사유 선택으로 이동 | 화면 3 | 인앱 (프로토타입 링크) |
...
|---|---|---|---|
| 사유 라디오 | 단일 선택 | — | — (이동 없음) |
| [해지 신청] | 완료 화면으로 이동 | 화면 4 | 인앱 (프로토타입 링크) |
...
| 미납요금 | 2 | |
| 잔여 할부금 | 2 | |
| 정산 합계 | 2 | 위 3개 단순 합인지 미확인 |
| 해지 사유 코드 | 3 | 4개 고정값. 코드 체계 미확인 |
| 해지 사유 기타 텍스트 | 3 | |
| 해지 예정일 | 4 | 서버 계산으로 추정 |
| 접수번호 | 4 | |
...
- [ ] 화면 2: 위약금·잔여 할부금은 어느 API에서 오나. 정산 합계는 클라이언트 합산인가 서버 계산인가
...
- [ ] 화면 2: [약관 전문 보기]가 어디로 가나. 별도 화면인가 바텀시트인가 외부 링크인가 — 인앱 라우팅인지 레거시 웹뷰 URL인지에 따라 작업이 완전히 다르다
